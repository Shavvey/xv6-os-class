...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3b5:	f7 f1                	div    %ecx
 3b7:	0f b6 92 fe 07 00 00 	movzbl 0x7fe(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5ce:	e9 53 ff ff ff       	jmp    526 <printf+0x106>
        ...
        if(s == 0)
          s = "(null)";
 5d3:	bf f7 07 00 00       	mov    $0x7f7,%edi
...
free(void *ap)
{
 ...
 5e1:	a1 10 0b 00 00       	mov    0xb10,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 61d:	89 08                	mov    %ecx,(%eax)
  freep = p;
 61f:	a3 10 0b 00 00       	mov    %eax,0xb10
}
 624:	5b                   	pop    %ebx
 625:	5e                   	pop    %esi
 626:	5f                   	pop    %edi
...
free(void *ap)
{
 ...
 653:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 656:	a3 10 0b 00 00       	mov    %eax,0xb10
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 679:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 67c:	8b 15 10 0b 00 00    	mov    0xb10,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
 ...
 6af:	76 4f                	jbe    700 <malloc+0x90>
    ...
    if(p == freep)
 6b1:	3b 05 10 0b 00 00    	cmp    0xb10,%eax
...
morecore(uint nu)
{
 ...
 6e8:	e8 f3 fe ff ff       	call   5e0 <free>
  return freep;
 6ed:	8b 15 10 0b 00 00    	mov    0xb10,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 70c:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 70f:	89 15 10 0b 00 00    	mov    %edx,0xb10
      return (void*)(p + 1);
