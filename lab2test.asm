...
_lab2test:     file format elf32-i386
...
int main(int argc, char *argv[]) {
  ...
  35:	53                   	push   %ebx
  36:	e8 d5 02 00 00       	call   310 <atoi>
      printf(1,"Setting new priority: %d\n", p);
  ...
  3e:	89 c3                	mov    %eax,%ebx
      printf(1,"Setting new priority: %d\n", p);
  40:	50                   	push   %eax
  41:	68 11 0a 00 00       	push   $0xa11
  46:	6a 01                	push   $0x1
  48:	e8 b3 04 00 00       	call   500 <printf>
      test_args(p);
  4d:	89 1c 24             	mov    %ebx,(%esp)
  50:	e8 8b 00 00 00       	call   e0 <test_args>
}
...
int main(int argc, char *argv[]) {
  ...
  if(argc <= 1) {
    test();
  55:	e8 16 00 00 00       	call   70 <test>
    char const *prior = argv[1];
    printf(1,"Passed priority: %s\n", prior);
    // dumb hacky nonsense to get negative numbers to parse correctly
    if(*prior == '-') {
  ...
  5d:	83 c3 01             	add    $0x1,%ebx
      int p = atoi(prior);
  60:	53                   	push   %ebx
  61:	e8 aa 02 00 00       	call   310 <atoi>
      // invert parsed int, it's stupid I know 
      p *= -1;
  66:	f7 d8                	neg    %eax
  68:	eb d1                	jmp    3b <main+0x3b>
  6a:	66 90                	xchg   %ax,%ax
  6c:	66 90                	xchg   %ax,%ax
  6e:	66 90                	xchg   %ax,%ax
...
void test() {
  ...
  86:	6a fe                	push   $0xfffffffe
  88:	68 f8 08 00 00       	push   $0x8f8
  8d:	6a 01                	push   $0x1
  8f:	e8 6c 04 00 00       	call   500 <printf>
  // get current process id 
  int pid = getpid();
  94:	e8 69 03 00 00       	call   402 <getpid>
  int res = set_priority(pid, new_priority);
  99:	5a                   	pop    %edx
  9a:	59                   	pop    %ecx
  9b:	6a fe                	push   $0xfffffffe
  9d:	50                   	push   %eax
void test() {
  printf(1,"Testing set_priority and get_priority syscalls!\n");
  int new_priority = -2;
  ...
  int res = set_priority(pid, new_priority);
  a0:	e8 7d 03 00 00       	call   422 <set_priority>
  if (res == 0) {
  a5:	83 c4 10             	add    $0x10,%esp
  a8:	85 c0                	test   %eax,%eax
  aa:	75 1f                	jne    cb <test+0x5b>
    int priority = get_priority(pid);
  ac:	83 ec 0c             	sub    $0xc,%esp
  af:	53                   	push   %ebx
  b0:	e8 75 03 00 00       	call   42a <get_priority>
  ...
  exit();
  c6:	e8 b7 02 00 00       	call   382 <exit>
  int res = set_priority(pid, new_priority);
  if (res == 0) {
    int priority = get_priority(pid);
      printf(1,"Process PID: %d now has newly set priority: %d\n",pid,priority);
...
void test_args(int new_priority) {
  ...
  int pid = getpid();
  e7:	e8 16 03 00 00       	call   402 <getpid>
  int res = set_priority(pid, new_priority);
...
void test_args(int new_priority) {
  ...
  int res = set_priority(pid, new_priority);
  f4:	50                   	push   %eax
  f5:	e8 28 03 00 00       	call   422 <set_priority>
  if(res == 0) {
  fa:	83 c4 10             	add    $0x10,%esp
  fd:	85 c0                	test   %eax,%eax
  ff:	75 1f                	jne    120 <test_args+0x40>
    int priority = get_priority(pid);
 101:	83 ec 0c             	sub    $0xc,%esp
 104:	53                   	push   %ebx
 105:	e8 20 03 00 00       	call   42a <get_priority>
      printf(1,"New priority %d for process PID: %d\n",priority,pid);
 10a:	53                   	push   %ebx
 10b:	50                   	push   %eax
 10c:	68 a8 09 00 00       	push   $0x9a8
 111:	6a 01                	push   $0x1
 113:	e8 e8 03 00 00       	call   500 <printf>
 118:	83 c4 20             	add    $0x20,%esp
  } else {
    printf(0, "Failed to get priority for process PID: %d\n", pid);
  }
  exit();
 11b:	e8 62 02 00 00       	call   382 <exit>
  int res = set_priority(pid, new_priority);
  if(res == 0) {
    int priority = get_priority(pid);
      printf(1,"New priority %d for process PID: %d\n",priority,pid);
 ...
 120:	50                   	push   %eax
 121:	53                   	push   %ebx
 122:	68 d0 09 00 00       	push   $0x9d0
 127:	6a 00                	push   $0x0
 129:	e8 d2 03 00 00       	call   500 <printf>
 12e:	83 c4 10             	add    $0x10,%esp
 131:	eb e8                	jmp    11b <test_args+0x3b>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 495:	f7 f1                	div    %ecx
 497:	0f b6 92 32 0a 00 00 	movzbl 0xa32(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 6ae:	e9 53 ff ff ff       	jmp    606 <printf+0x106>
        ...
        if(s == 0)
          s = "(null)";
 6b3:	bf 2b 0a 00 00       	mov    $0xa2b,%edi
...
free(void *ap)
{
 ...
 6c1:	a1 74 0d 00 00       	mov    0xd74,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 6fd:	89 08                	mov    %ecx,(%eax)
  freep = p;
 6ff:	a3 74 0d 00 00       	mov    %eax,0xd74
}
 704:	5b                   	pop    %ebx
 705:	5e                   	pop    %esi
 706:	5f                   	pop    %edi
...
free(void *ap)
{
 ...
 733:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 736:	a3 74 0d 00 00       	mov    %eax,0xd74
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 759:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 75c:	8b 15 74 0d 00 00    	mov    0xd74,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
 ...
 78f:	76 4f                	jbe    7e0 <malloc+0x90>
    ...
    if(p == freep)
 791:	3b 05 74 0d 00 00    	cmp    0xd74,%eax
...
morecore(uint nu)
{
 ...
 7c8:	e8 f3 fe ff ff       	call   6c0 <free>
  return freep;
 7cd:	8b 15 74 0d 00 00    	mov    0xd74,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 7ec:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
 7ef:	89 15 74 0d 00 00    	mov    %edx,0xd74
      return (void*)(p + 1);
...
morecore(uint nu)
{
 ...
 814:	eb d9                	jmp    7ef <malloc+0x9f>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 816:	c7 05 74 0d 00 00 78 	movl   $0xd78,0xd74
 81d:	0d 00 00 
 820:	c7 05 78 0d 00 00 78 	movl   $0xd78,0xd78
 827:	0d 00 00 
    base.s.size = 0;
 82a:	b8 78 0d 00 00       	mov    $0xd78,%eax
 82f:	c7 05 7c 0d 00 00 00 	movl   $0x0,0xd7c
 836:	00 00 00 
